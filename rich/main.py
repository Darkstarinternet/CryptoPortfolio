...
class CryptoPortfolioRich:
    ...
    def create_portfolio_dictionary(self):
        ...
        btc_price = \
            response_dictionary['data']['BTC'][0]['quote']['USD']['price']

        for key in self.holdings_dictionary:
            crypto_name = response_dictionary['data'][key][0]['name']
            crypto_price_float = \
                response_dictionary['data'][key][0]['quote']['USD']['price']
            try:
                holding_amount_float = float(self.holdings_dictionary.get(key))
            except ValueError:
                self.console.print(f"Error in create_crypto_dictionary(): "
                                   f"Invalid value in holdings_dictionary "
                                   f"for key: {key}")
                sys.exit()

            # calculate holdings as floats
            holding_usd_value_float = holding_amount_float * crypto_price_float
            holding_gbp_value_float = (holding_amount_float *
                                       crypto_price_float) * usd_gbp_rate
            holding_btc_value_float = holding_usd_value_float / btc_price

            # convert holdings from floats to strings and format them
            holding_usd_value_string = f"{holding_usd_value_float:.2f}"
            holding_gbp_value_string = f"{holding_gbp_value_float:.2f}"
            holding_btc_value_string = f"{holding_btc_value_float:.4f}"
    ...
    def print_portfolio_table(self):
        ...
        usd_total_string = f"{usd_total_float:.2f}"
        btc_total_string = f"{btc_total_float:.4f}"
        gbp_total_string = f"{gbp_total_float:.2f}"
        ...
        for key, value in self.portfolio_data_dictionary.items():
            ...
            symbol_string = key
            amount_string = f"{value['Amount']:.4f}"
            current_price_string = f"${value['Current Price']:.5f}"
